...
export default function App() {
  ...
  // 🧠 Preload received images with better error handling
  const preloadPendingImages = () => {
    ...
    imagesToProcess.forEach((base64) => {
      ...
      img.onload = () => {
        ...
        // If this is the first frame and we're not playing yet, show it
  ...
  // 📦 Smart buffer management - request more frames when needed
  const checkBufferAndRequest = () => {
    const bufferSize = frameCountRef.current - currentIndexRef.current;
    const minBuffer = Math.max(10, Math.ceil(30 * speed)); // Adaptive buffer size
    ...
    if (bufferSize < minBuffer && isStreamingRef.current && !hasRequestedMoreRef.current) {
      ...
      // Reset the request flag after a short delay to allow for multiple requests
      setTimeout(() => {
  ...
  // ▶️ Smooth playback loop using requestAnimationFrame
  const playLoop = (currentTime: number) => {
    ...
    if (currentTime - lastFrameTimeRef.current >= frameInterval) {
      ...
      if (nextIndex < framesRef.current.length) {
        ...
        // Check buffer and request more frames if needed
        checkBufferAndRequest();
      } else {
        // No more frames available - check if stream is still active
        if (isStreamingRef.current) {
          setIsBuffering(true);
          checkBufferAndRequest();
        } else {
          // Stream ended, stop playing
          setIsPlaying(false);
          return;
        }
      }
    ...
    <div className="min-h-screen bg-black flex flex-col items-center justify-center p-5">
      ...
      <div className="flex items-center gap-4 mt-4 text-white">
        ...
        <button
          ...
          className="px-4 py-2 bg-blue-600 rounded hover:bg-blue-700 transition flex items-center gap-2"
